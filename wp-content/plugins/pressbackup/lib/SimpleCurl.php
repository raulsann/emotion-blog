...
class SimpleCurlPBLib
{
	...
	public function call($args)
	{
		...
		*/
		...
		curl_setopt($curl_handle, CURLINFO_HEADER_OUT, $args['return.header.out'] );
		...
		if (isset($args['upload.file.PUT'])) {
			...
			if($this->Misc){
				$this->Misc->writeLogFile('sent.percent', '0|'.$this->tmp_read_file_size);
			}
		}
		...
		*/
		...
		if($args['return.header.out']){
			$this->response['body'] = curl_getinfo($curl_handle, CURLINFO_HEADER_OUT) . $this->response['body'];
		}
	...
	private function __responseReadCallback(&$curl, $fp, $len)
	{
		...
		if($this->Misc){
			$this->Misc->writeLogFile('sent.percent', @ftell($this->tmp_read_file).'|'.$this->tmp_read_file_size);
		}
	...
	private function parseResponse ($type)
	{
		...
		} elseif (in_array($type, array('auto', 'json')) && $json =  @json_decode($this->buffer, true) ) {
